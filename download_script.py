...
def concat_video():
	...
	code = f"type *.{filetype} > ../../Video/merged.{filetype}"

	open(temp_dir + '/concat_video.bat', 'w').write(code)
	print('Concatenating video files...')
	os.chdir(temp_dir)
	open_bat = Popen(['concat_video.bat'])
	open_bat.wait()
	if open_bat.returncode == 0:
		os.chdir('../../../')
...
def concat_audio():
	...
	code = f"type *.{filetype} > ../../Audio/merged.{filetype}"

	open(temp_dir + '/' + 'concat_audio.bat', 'w').write(code)
	print('Concatenating audio files...')
	os.chdir(temp_dir)
	open_bat = Popen(['concat_audio.bat'])
	open_bat.wait()
	if open_bat.returncode == 0:
		os.chdir('../../../')
		shutil.rmtree('Downloads/temp')
...
def convert_video_audio():
	...
	open('ffmpeg_convert.bat', 'w').write(code)
	print('Starting to convert...')
	subprocess.run('ffmpeg_convert.bat')
	os.remove('ffmpeg_convert.bat')
